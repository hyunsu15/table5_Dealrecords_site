...
import {
  CreatePayPerson_Mutation,
  UpdateIsAllPay,
  ChangeFalseIsDeliver,
  ChangeTrueIsDeliver
...
function ModifyPayPerson() {
  ...
  function _controller() {
    if (choose == 0)
      return (
        <CreateBankInfo.Provider value={[bank, setBank]}>
  ...
  function _saveClick() {
    ...
    if (choose == 0) {
      ...
      ID.then(function(result) {
        updateIsAllPay({
          variables: { PayPerson: result.data.createPayPerson.ID, Deal: DealID }
        }); //만든 결제자 ID
      });
    }
    if (choose == 1) {
      updateIsAllPay({
        variables: { PayPerson: connectPayPerson, Deal: DealID }
      });
    }
  }
    ...
    <React.Fragment>
      ...
      <Paper>
        <ConnectPayPersonID.Provider
          value={[connectPayPerson, setConnectPayPerson]}
        >
          {_controller()}
          <Grid
            container
