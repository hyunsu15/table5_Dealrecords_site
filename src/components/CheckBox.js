...
import {Store,ModifyDealID,optimalRequest,v1Info} from "../Store"; 
 ...
 const CheckBox=({isDeliver,isAllPay,ID})=>{
    ...
    const _ChangeState=(isDeliver,isAllPay)=>{
     ...
     if(!isAllPay&&checkBox.value2) { _isTrue();return setAppChoose(2)}
    if(isAllPay&&checkBox.value2) changeFalseAllPay({variables:{ID:ID}})
    if(!isDeliver&&checkBox.value1) changeTrueDeliver({variables:{ID:ID}})
    if(isDeliver&&checkBox.value1) changeFalseDeliver({variables:{ID:ID}})
...
const checkBoxList=(checkBox,setCheckBox,fx) =>{
            ...
            <Grid container justify="center">
                        ...
                        결제반전<input type="checkbox" name="checkbox1" value="결제" onChange={() => { setCheckBox({...checkBox,value2: !checkBox.value2})}}/>
                        <Button style={{ fontWeight: "bold" }} onClick={()=>fx()}>저장</Button>
                        </Box>
                    </Grid>);
}
...
/**        {isDeliver!=staticCheckBox.originV1&&setStaticCheckBox({...staticCheckBox,originV1:isDeliver})}
            {checkBox.value1!=staticCheckBox.value1&&setStaticCheckBox({...staticCheckBox,value1:checkBox.value1})}
            {isAllPay!=staticCheckBox.originV2&&setStaticCheckBox({...staticCheckBox,originV2:isAllPay})}
            {checkBox.value2!=staticCheckBox.value&&setStaticCheckBox({...staticCheckBox,value2:checkBox.value2})}
     */