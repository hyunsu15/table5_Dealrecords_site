...
const Main = () => {
  ...
  const _PaperQuery = Query_name => {
    return (
      <Query query={Query_name} pollInterval={100}>
        {({ loading, error, data }) => {
            ...
            <GridList>
              {data.deals.map(record => (
                <Deal
                  ...
                  userPhonNumber={record.user.userPhonNumber}
                  payPerson={record.payPerson.payPerson}
                  productName={record.product.productName}
                  productNum={record.productNum}
                  productCost={record.productCost}
                  isAllPay={record.payPerson.isAllPay}
                  isDeliver={record.isDeliver}
                  bank={record.payPerson.bank}
                />
              ))}
            </GridList>
          );
...
const _connectDbAdmin = choose => {
  if (choose == 3)
    window.open(
      'https://db-layer-de1a8af52f.herokuapp.com/data-layer/dev/_admin',
      '_blank'
    );
};
